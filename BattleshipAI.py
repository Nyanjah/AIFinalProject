...
class SimpleBattleship(object):
    ...
    def __init__(self, shipCoords, numBoardRows, numBoardCols):
        ...
        # where the shooting is pointing at
        self.board = np.zeros(shape=[self.numBoardRows+1, self.numBoardCols])
        self.shootingCoord = 0 # shooting always starts at left most pos
        self.updateshootingCoord(self.shootingCoord)

    def updateshootingCoord(self, coord, val=1):
        # reset board
        self.board[self.numBoardRows,range(self.numBoardCols)] = 0
        # show new shooting location
    ...
    def fireShot(self, shotCoordinate):
        ...
        if shotCoordinate in self.shotsTakenSofar:
            self.updateshootingCoord(shotCoordinate)
            ...
            if not 0 <= shotCoordinate <= self.numBoardCols-1:
                # Invalid
                # update shooting to be outside board = all zero
                self.updateshootingCoord(range(0,self.numBoardCols), 0)
                return self.getOneDimState(), RewardFun[SHOT_RESULT.INVALID], GAME_STATUS.COMPLETE

            elif shotCoordinate in self.shipCoords:
                # Hit
                self.updateState(shotCoordinate, 1)
                # The shooting moves to where we fired at.
                self.updateshootingCoord(shotCoordinate)
...
ship = [random.randint(0,7),random.randint(0,7)]
...
inputs1 = tf.Variable(tf.ones(shape=[1,BOARD_SIZE[1]*2]), dtype=tf.float32)
W = tf.Variable(tf.random.uniform([BOARD_SIZE[1]*2,len(actionSet)],0,0.01))
Qout = tf.matmul(inputs1,W)
predict = tf.argmax(Qout,1)

#Below we obtain the loss by taking the sum of squares difference between the target and prediction Q values.
nextQ = tf.Variable(initial_value = range(1,len(actionSet)), shape=[1,len(actionSet)],dtype=tf.float32)
...
with tf.Session() as sess:
    ...
    for i in range(NUM_EPISODES):
        ...
        print(game.getState())
        while j < 60: #should never need more than 49 (max number of legal moves if board size is 7x7)
            j+=1
            ...
            shooting += actionSet[actionIdx[0]]

            s1, r, gameStatus = game.fireShot(shooting)
            print(str(j)+"\n")
            print(game.getState())
            ...
            #Save the new weights to file
            saver.save(sess, './Battleship-models/model.ckpt')
